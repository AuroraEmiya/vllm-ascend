...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    //store_kv_block
    ops.def(
        "store_kv_block_pre(Tensor slot_mapping_npu, int[] slot_mapping_list =[], int block_size=0)"
    ...
    ops.def(
        "store_kv_block(Tensor key_in, Tensor key_cache_in, Tensor group_len, Tensor group_key_idx, "
    ...
    // store kv decode
    ops.def(
        "store_kv_decode(Tensor key_in, Tensor key_cache_in, Tensor slot_mapping_list) -> ()"
    );
