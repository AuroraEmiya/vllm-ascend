...
#ifndef read_kv_cache_TORCH_ADPT_H
#define read_kv_cache_TORCH_ADPT_H
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> read_kv_cache_pre(const at::Tensor& slot_mapping_npu,
  ...
  while (key_idx_slotmap < slot_mapping_len) {
    ...
    // cur_key_outdex = block_id * block_size + block_offset
  ...
  TORCH_CHECK(ret == ACL_SUCCESS, "read_kv_cache_pre copy group_len failed, ret=", ret);

  ret = aclrtMemcpyAsync(group_key_idx_addr, device_size, &key_idx[0], device_size, memcpy_type, stream);
  TORCH_CHECK(ret == ACL_SUCCESS, "read_kv_cache_pre copy group_key_idx failed, ret=", ret);

  ret = aclrtMemcpyAsync(group_key_cache_idx_addr, device_size, &key_cache_idx[0], device_size, memcpy_type, stream);
  TORCH_CHECK(ret == ACL_SUCCESS, "read_kv_cache_pre copy group_key_cache_idx failed, ret=", ret);

  // ret = aclrtSynchronizeStream(stream);
  // TORCH_CHECK(ret == ACL_SUCCESS, "read_kv_cache_pre synchronize stream failed, ret=", ret);
