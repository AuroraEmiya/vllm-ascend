...
std::tuple<at::Tensor, at::Tensor, at::Tensor> store_kv_decode_pre(const at::Tensor& slot_mapping_npu,
  ...
  while (key_idx_slotmap < slot_mapping_len) {
    ...
    length[idx_groups] = static_cast<int32_t>(1);
  ...
  TORCH_CHECK(ret == ACL_SUCCESS, "store_kv_decode_pre copy group_len failed, ret=", ret);

  ret = aclrtMemcpyAsync(group_key_idx_addr, device_size, &key_idx[0], device_size, memcpy_type, stream);
  TORCH_CHECK(ret == ACL_SUCCESS, "store_kv_decode_pre copy group_key_idx failed, ret=", ret);

  ret = aclrtMemcpyAsync(group_key_cache_idx_addr, device_size, &key_cache_idx[0], device_size, memcpy_type, stream);
  TORCH_CHECK(ret == ACL_SUCCESS, "store_kv_decode_pre copy group_key_cache_idx failed, ret=", ret);

  // ret = aclrtSynchronizeStream(stream);
  // TORCH_CHECK(ret == ACL_SUCCESS, "store_kv_decode_pre synchronize stream failed, ret=", ret);

  return std::tuple<at::Tensor, at::Tensor, at::Tensor>(group_len, group_key_idx, group_key_cache_idx);
}

inline void store_kv_decode(const at::Tensor& key_in, const at::Tensor& key_cache_in, const at::Tensor& group_len,
                           const at::Tensor& group_key_idx, const at::Tensor& group_key_cache_idx, int64_t block_size) {
  EXEC_NPU_CMD(aclnnStoreKVDecode, key_in, key_cache_in, group_len, group_key_idx, group_key_cache_idx, block_size);
}
