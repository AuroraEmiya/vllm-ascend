...
def assert_store_kv_decode_registered():
    assert hasattr(torch.ops, "_C_ascend"), "torch.ops._C_ascend is not registered"
    assert hasattr(
        torch.ops._C_ascend,
        "store_kv_decode_pre",
    ), "torch.ops._C_ascend.store_kv_decode_pre is not registered"
    assert hasattr(
        torch.ops._C_ascend,
        "store_kv_decode",
    ), "torch.ops._C_ascend.store_kv_decode is not registered"
...
def test_storeKvDecode_with_continuous(
    num_tokens,
    num_head,
    num_blocks,
    head_size,
    ...
    assert_store_kv_decode_registered()
    ...
    for _ in range(epoch):
        group_len, group_key_idx, group_key_cache_idx = torch.ops._C_ascend.store_kv_decode_pre(
        slot_mapping_npu,
        slot_mapping_list,
        block_size,
    )
        ...
        torch.ops._C_ascend.store_kv_decode(
            keylist_npu,
            cache_table_npu,
            group_len,
            group_key_idx,
...
def test_storeKvDecode_without_continuous(
    num_tokens,
    num_head,
    num_blocks,
    head_size,
...
def test_storeKvDecode_without_ascending(
    num_tokens,
    num_head,
    num_blocks,
    head_size,
...
#         group_len,group_key_idx,group_key_cache_idx=torch.ops._C_ascend.store_kv_decode_pre( slot_mapping_npu, slot_mapping_list, block_size)
#         torch.ops._C_ascend.store_kv_decode(key_npu, key_cache_npu, group_len, group_key_idx, group_key_cache_idx, block_size)
#     N = 100
