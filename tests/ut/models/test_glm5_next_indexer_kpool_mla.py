...
@patch("torch.ops._C_ascend.npu_scatter_nd_update_v2", create=True)
def test_indexer_kpool_mla_state_write_keeps_negative_slots_for_device_filter(mock_scatter):
    ...
    assert scatter_slots.tolist() == [[2], [-1]]
...
def test_indexer_kpool_mla_full_decode_avoids_dynamic_topk_and_cpu_length(
    ...
    def scatter_with_negative_slot_filter(cache, indices, updates):
        for row, index in enumerate(indices.tolist()):
            index = tuple(index)
            if all(0 <= value < cache.shape[dim] for dim, value in enumerate(index)):
                cache[index] = updates[row]

    mock_scatter.side_effect = scatter_with_negative_slot_filter
